...
# 7 --> 23
# 6 --> 22
# equation is: for i < 16: i + offset => new index
# equation is: for i >= 16: (i + offset) % 26 => new index
    ...
    for word in split_text:
        ...
        for char in word:
            if char in alphabet:
                ...
                #print(char_index)
                if char_index < len(alphabet) - offset:
                    new_index= char_index + offset
                    new_word+= alphabet[new_index]
                elif char_index >= len(alphabet) - offset:
                    new_index= (char_index + offset) % 26
                    new_word+= alphabet[new_index]
            else: # for commas, periods, and exclamation points, etc.
                new_word+= char
...
print(decoder(vishals_message, 10))


# In[39]:


# test cases for decoder() with offset 10.
# test one word: xuo => hey
print(decoder('xuo', 10))
# test one word with non-alphabetical character: xuo! ==> hey!
print(decoder('xuo!', 10))
# test two words with a space: xuo jxuhu ==> hey there
print(decoder('xuo jxuhu', 10))
# test sentence: xuo jxuhu! jxyi yi qd unqcfbu ev q squiqh syfxuh. ==> ?
sentence= "xuo jxuhu! jxyi yi qd unqcfbu ev q squiqh syfxuh."
print(decoder(sentence, 10))
...
# test cases for coder() with offset 10.
print((7-10) % 26)
# test one word: hey => xuo
print(coder('hey', 10))
# test one word with non-alphabetical character: hey! ==> xuo!
print(coder('hey!', 10))
# test two words with a space: hey there ==> xuo jxuhu
print(coder('hey there', 10))
# test sentence: this is an example of a caesar cipher. ==> ?
sentence= "this is an example of a caesar cipher."
...
first_message= "the offset for the second message is fourteen."
print(coder(first_message, 10))
second_message= "performing multiple caesar ciphers to code your messages is even more secure!"
print(coder(second_message, 14))
...
def no_shift_decoder(text):
    for i in range(len(alphabet)):
        print(decoder(text, i))
    return
no_shift_decoder(text)
        ...
        for char in word:
            if char not in alphabet:
                ...
                if counter >= len(keyword):
                    counter= 0
                new_char_index= alphabet.index(char) - alphabet.index(keyword[counter])
...
print(vigenere_decoder(vishals_message, vishals_keyword))
...
print(vigenere_coder('barry', 'dog'))
vishals_message= "you were able to decode this? nice work! you are becoming quite the expert at crytography!"
print(vigenere_coder(vishals_message, vishals_keyword))
print(vigenere_decoder(vigenere_coder(vishals_message, vishals_keyword), vishals_keyword))
